...
contract StablePriceOracle is Initializable, Ownable, PriceOracle {
    ...
    function __StablePriceOracle_i(
    ...
    function __StablePriceOracle_init(
        AggregatorInterface _usdOracle,
        uint256[] memory _rentPrices
    ...
    function __StablePriceOracle_init_unchained(
        AggregatorInterface _usdOracle,
        uint256[] memory _rentPrices
    ) internal onlyInitializing {
        usdOracle = _usdOracle;
    ...
    function price(
        ...
        return attoUSDToWei(basePrice.add(_premium(name, expires, duration)));
    }
    ...
    function attoUSDToWei(uint amount) public pure override returns (uint) {
        // uint ethPrice = uint(usdOracle.latestAnswer());
        uint ethPrice = 179651394650;
